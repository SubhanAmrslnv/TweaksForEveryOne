...
### Custom Taskbar Clock — the only network egress

A clock overlay: `CustomClockEnabled`, ini key `[taskbar] customclock`, wired through `LoadSettings` / `WriteSettings` / `BuildWin` / `ApplyUi`, armed from the deferred-init block by `SyncCustomClockTimer()` and driven at 1 s by `UpdateCustomClock()`. It puts a click-through GUI over each taskbar's native clock control and rewrites it as time / date / temperature.

**One overlay per taskbar, one timer for all of them.** `CustomClockBars` is a Map keyed by taskbar HWND holding `{gui, text, clock, rect, fit, lean}`; `CustomClockTaskbars()` enumerates `Shell_TrayWnd` plus every `Shell_SecondaryTrayWnd`; `UpdateCustomClock()` prunes dead bars (walking a `.Clone()`, since `DestroyClockBar` deletes from the Map), calls `UpdateClockBar()` per bar inside its own `try` so a broken secondary cannot take the primary down, and then flushes **once** for all of them. Every piece of remembered state is per-bar because two monitors can differ in DPI, taskbar thickness and clock width — there is no shared answer to cache. Do not add a second timer for this.

**The clock control class differs between taskbars.** Measured on 26200: the primary `Shell_TrayWnd` has `TrayClockWClass1`, and every `Shell_SecondaryTrayWnd` has **`ClockButton1`** instead — same 54×30 box, different class. Searching only for the first is why the feature did nothing at all on a second display. `CLOCK_CLASSES` holds both and `FindTrayClock(tbHwnd, rec)` caches the resolved HWND on that bar's record.

**`ABM_GETTASKBARPOS` only ever describes the primary bar**, so `TaskbarMetrics()` checks `WinGetClass(tbHwnd) = "Shell_TrayWnd"` first and sends secondary bars down the geometric path — otherwise a secondary bar inherits the primary's edge and thickness, which is the wrong answer on a mixed-DPI or differently-docked setup.

**`FetchWeather()` performs an outbound HTTPS request** — `ComObject("Msxml2.ServerXMLHTTP")` against `https://wttr.in`, async, refreshed every 15 minutes, with the response written into `CustomClockWeather` by `WeatherCallback`. This is the **only** network call in the program, and the "nothing is written outside the program folder" paragraph above was written when none existed. There is no opt-out beyond the feature's own checkbox. If privacy or offline behaviour matters, this is the one place to look. The COM object is held in the global `CustomClockReq` specifically so it is not garbage-collected mid-flight — do not "tidy" that into a local — and `FetchWeather` allows one request at a time, abandoning a stalled one after 60 s.

Five measured behaviours the current shape depends on. Each of these was a bug that put nothing, or the wrong thing, on screen:

- **`ControlGetHwnd`'s first argument is a ClassNN, not a class name.** `ControlGetHwnd("TrayClockWClass", ...)` **throws** `Target control not found` on the same window where `WinGetControls` lists `TrayClockWClass1`. This is why the feature never rendered at all. `FindTrayClock()` resolves the instance number by scanning and caches the HWND until the window dies. A 0 return is the normal case on a **stock Win11 taskbar**, whose XAML clock is not a `TrayClockWClass` — the overlay then correctly does nothing.
- **Never request `WS_EX_LAYERED` (`+E0x80000`) in a `Gui()` option string.** A window carrying it with no matching `SetLayeredWindowAttributes` call draws *nothing*, so asking for it is a way to *cause* an invisible overlay. `RS_SetAlpha` adds the style and sets the attribute together. `WS_EX_TRANSPARENT` (`+E0x20`) has never been a reason a window fails to render — under DWM it only affects hit-testing and sibling paint order.
- **The overlay must re-assert `HWND_TOPMOST` every tick.** `Shell_TrayWnd` is `WS_EX_TOPMOST` too, and within the topmost band z-order is insertion order, so the taskbar raising itself on hover / click / Win buries the overlay permanently. Measured: overlay at z-index 13 vs taskbar 19, then 17 vs 16 after the taskbar raised itself, then 13 vs 17 again one tick later. Nothing else put it back.
- **`Msxml2.XMLHTTP` silently ignores `setRequestHeader("User-Agent", ...)`.** wttr.in picks plain text vs its HTML page from the UA, so the same request returns 11,953 bytes of `<!DOCTYPE html>` through `Msxml2.XMLHTTP` (with or without the header) and 17 bytes of `+33C` through `Msxml2.ServerXMLHTTP` with it — and the overlay painted the HTML over the taskbar. `ServerXMLHTTP` supports the same async `onreadystatechange` interface. `WeatherCallback` additionally rejects any answer that is empty, over 40 chars, or contains `<`, so a remote response can never be painted verbatim.
- **Taskbar shape is measured, never read from the registry.** `TaskbarMetrics()` returns `{edge, thick, logical, small}` from `ABM_GETTASKBARPOS` (`GetTaskbarPos()`, beside the two existing `SHAppBarMessage` wrappers) with a geometric fallback, DPI-normalised through `DpiForWindow()`. The old `RegRead(ADVANCED_KEY, "TaskbarSmallIcons")` threw when the value was absent and said nothing about a non-ExplorerPatcher taskbar mod. Measured logical thicknesses on 26200: Win11 native 48, Win10-style large 40, Win10-style small 30-32, so `<= 36` reads as small and selects a two-line layout instead of three. `edge` decides which way the overlay grows so it never runs off the screen.

**Visibility is one containment test, not four coordinate tests.** `MonitorRectAt()` gives the rect of the monitor holding the taskbar, and the overlay hides whenever the *native clock's* rect leaves it. That covers all four edges, auto-hide, the mid-slide animation and a bar the user has dragged, because when the bar goes the clock goes with it. Do not reintroduce a comparison against `A_ScreenWidth`/`A_ScreenHeight`: those describe the primary monitor, so a taskbar on a secondary monitor to the left sits at negative x and reads as permanently hidden.

#### The overlay must be exactly the clock's rect, and the text fitted to it

**The tray children abut at gap 0 and share one vertical centre.** Measured on 26200 at DPI 96, left to right: `SysPager1`/`ToolbarWindow32` (tray icons) → `TrayInputIndicatorWClass1` (the `ENG`/`AZE` language button, 36px) → `ControlCenterButton1` (the gear, 32px) → `TrayClockWClass1` (54px) → `TrayButton1` (notifications, 48px) → `TrayShowDesktopButtonWClass1` (5px, ending exactly at the screen edge by design). **Every gap is 0, every height is the full taskbar height, and every `centreY` is identical.** Windows lays this out; nothing in this program positions any of it.

Two consequences, both of which have already been got wrong:

- **Any overlay wider than the control it covers necessarily eats a neighbour.** Widening the clock overlay to a "readable minimum" and growing it leftwards painted its opaque background over 11px of `ControlCenterButton1`, which reads as Windows having mispositioned its own gear icon. `UpdateCustomClock` therefore uses `cx, cy, cw, ch` verbatim — the clock's own box, never `Max(cw, n)`. Doing so also inherits the native right boundary and the native vertical centre for free, which is why there is no per-DPI or per-resolution boundary arithmetic anywhere in this feature.
- **Do not read a native tray misalignment into a zoomed screenshot.** The show-desktop strip is 5px wide and ends at the screen edge, so a magnified crop bounded by the screen edge makes it look like a clipped icon.

**Text is fitted to the box by measurement, never by a font constant.** `MeasureTextWidth` / `MeasureTextHeight` call `GetTextExtentPoint32W` against the control's own DC with its own font (fetched with `WM_GETFONT` by `DllCall`, not AHK's `SendMessage`, whose WinTitle resolution does not reliably find a child control under `DetectHiddenWindows false`). `FitCustomClockText(lines, w, h, minSize)` walks sizes largest-first and returns the one that fits, or 0. A points-to-pixels ratio is not a constant — it depends on the DPI and on the font Windows actually substituted — so nothing here is safe to hard-code.

**Content degrades in a fixed order, and the date is never what goes.** `dd.MM.yyyy` is produced by an explicit `FormatTime` pattern, so separators and the leading zeros on day and month do not depend on regional settings. The temperature shares the *time* line, never the date line. If time+temperature will not fit at the s6 legibility floor, the temperature is dropped and time+date stay (`CustomClockLean`); a bar with more room fits all three lines. Note that **Windows' own clock omits the date entirely on a small taskbar** — `ControlGetText` on `TrayClockWClass1` returns just `"System Clock, 13:52:27"` — so a missing date there is native behaviour at that size, not a geometry bug to chase.

The re-fit is keyed on box size + layout + weather-string length (`rec.fit`), so ticking seconds never re-run seven font measurements on every monitor.

One AHK v2 trap this feature hit twice, worth stating because the symptom was invisible: **a name assigned inside a function must be declared `global`, or it silently becomes a local** — and the read that comes first then throws "unset variable" straight into the function's own `catch`, which presented as the overlay simply never appearing, with no error anywhere. That is why per-bar state now lives on the `rec` object instead of in globals: an object field cannot be shadowed by a missing declaration.
